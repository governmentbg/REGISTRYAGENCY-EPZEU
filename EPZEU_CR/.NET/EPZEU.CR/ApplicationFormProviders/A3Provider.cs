...
{
    /// <summary>
    /// Реализация на интерфейс IApplicationAProvider за работа със съдържанието на заявление "А3 Заявление за вписване на обстоятелства относно командитно дружество"
    /// </summary>
    internal class A3Provider : ApplicationFormAProviderBase<A3>
    {
        public override SubUICTypes SubUICType => SubUICTypes.MainCircumstances;

        public override LegalForms LegalFormBase => LegalForms.KD;
       ...
       protected override async Task<CNSys.OperationResult> InitApplicationInternalAsync(ApplicationInitParameters initParams)
        {
            ...
            #region LiabilityPartners

            if (application.Fields.LimitedLiabilityPartners == null)
            {
                application.Fields.LimitedLiabilityPartners = new F021_LimitedLiabilityPartners();
            }

            if (application.Fields.LimitedLiabilityPartners.LiabilityPartnerList == null)
            {
                application.Fields.LimitedLiabilityPartners.LiabilityPartnerList = new List<F0210_LimitedLiabilityPartner>();
            }

            //Трябва да има поне един съдружник.
            if (application.Fields.LimitedLiabilityPartners.LiabilityPartnerList.Count == 0)
            {
                var partner = new F0210_LimitedLiabilityPartner();

                application.Fields.LimitedLiabilityPartners.LiabilityPartnerList.Add(partner);
            }
