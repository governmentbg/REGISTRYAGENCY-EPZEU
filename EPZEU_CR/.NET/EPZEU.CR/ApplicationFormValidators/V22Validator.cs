...
{
    internal class V22Validator : VBaseValidator<V22>
    {
        ...
        protected override IErrorCollection ValidateInternal(V22 application, bool isMainApplication = true)
        {
            ...
            if (application != null && application.Fields != null)
            {
                ...
                if (application.Fields.TransformingCompanys != null && application.Fields.TransformingCompanys.TransformingCompanyList != null && 
                    application.Fields.TransformingCompanys.TransformingCompanyList.Count > 0 && application.Fields.TransformingCompanys.TransformingCompanyList[0].Subject != null)
                {
                    var additinalAppsForChangeForTransferringEnterpriseIdCount = additionalApps.Where(x => x.ApplicationState == ProcessStates.ForChange
                    && x.GetFiledsContainer() != null && x.GetFiledsContainer().UIC != null 
                    && x.GetFiledsContainer().UIC.Text == application.Fields.TransformingCompanys.TransformingCompanyList[0].Subject.Indent).Count();

                    MatchedAdditionalApplicationsCount += additinalAppsForChangeForTransferringEnterpriseIdCount;
                }
