...
{
    public class DocumentsWithoutDeedSearchCriteriaValidator : BasePagedSearchCriteriaValidator<DocumentsWithoutDeedSearchCriteria>
    {
        public DocumentsWithoutDeedSearchCriteriaValidator(IOptionsMonitor<GlobalOptions> globalOptionsAccessor) : base(globalOptionsAccessor)
        {
        }

        public override ValidationResult Validate(ValidationContext<DocumentsWithoutDeedSearchCriteria> context)
        {
            var result = base.Validate(context);
            var model = context.InstanceToValidate;

            if((model.DateFrom == null || model.DateTo == null)
                && string.IsNullOrEmpty(model.IncomingNumber.Trim())
                && string.IsNullOrEmpty(model.CompanyName.Trim())
                && model.Status == null)
            {
            ...
            else
            {
                ...
                {
                    if(!string.IsNullOrEmpty(model.IncomingNumber.Trim()) || !string.IsNullOrEmpty(model.CompanyName.Trim()) || model.Status != null)
                    {
