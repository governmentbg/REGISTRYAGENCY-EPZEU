import { ApiError, ArrayHelper, ClientError, handleErrorLog, moduleContext, TypeSystem, ObjectHelper } from "Cnsys.Core";
...
export function withAsyncFrame<C extends React.ComponentClass<any>>(Component: C, showError: boolean = true, withLocalLoader: boolean = false): C {
                    ...
                    {errors.map((err, idx) => {
                        return (
                            <span key={idx}>
                                ...
                                {idx < errors.length - 1 && errors.length > 1 ? <br /> : null}
                            </span>);
                    })}
                </div>);
        }
                ...
                <div className="alert alert-warning">
                    {warnings.map((err, idx) => {
                        return (
                            <span key={idx}>
                                {this.getMessageFromError(err)}
                                {idx < warnings.length - 1 && warnings.length > 1 ? <br /> : null}
                            </span>);
                    })}
                </div>);
        }
