...
import { observable, action, runInAction, autorun, IReactionDisposer, reaction, intercept } from "mobx";
import { observer } from "mobx-react";
import { BindableReference, IModelErrors, IBaseValidator, ObjectHelper, Helper, TypeSystem, ResourceHelpers, ArrayHelper } from 'Cnsys.Core';
import { TextBox, CheckBox, CheckBoxList, DropDownList, Label, RadioButtonList, TextArea, Date, DateTime, Hint, RawHTML, Password, Amount, Time, Duration } from './CommonFields'
...
export class BaseComponent<TProps extends BaseProps, TModel extends IModelErrors> extends React.Component<TProps, any> {
    ...
    protected timeFor(selector: (model: TModel) => any, attributes?: any, onChange?: (event: any, value: any, modelReference: BindableReference) => void, showSeparateDateAndTime?: boolean): any {
        return <Time {...this.bind(selector) } attributes={attributes} onChange={onChange} showSeparateHoursAndMinutes={showSeparateDateAndTime} />;
    }
    ...
    protected rawHtmlFor(selector: (model: TModel) => any, text?: string, attributes?: any): any {
        return <RawHTML {...this.bind(selector) } rawHtmlText={text} attributes={attributes} />
    }
