...
import { V31 } from '../ApplicationFormsV';
import { ApplicationFormVValidator } from './ApplicationFormVValidator';
import { V31FieldsValidator } from './V31FieldsValidator';

export class V31Validator extends ApplicationFormVValidator<V31> {
    private _matchedAdditionalA7ApplicaitonsCount: number = 0;

    constructor() {
        super();

        this.ruleFor(m => m.fields).setValidator(new V31FieldsValidator());
    }

    public validate(obj: V31): boolean {
    ...
    private validateAdditionalApps(mainApp: V31): boolean {
        ...
        let errors = this.getErrorsForAppsForSuccessor(mainApp.fields.successors803.successorList[0], a7List, mainApp.fields.formOfTransforming801.influx); // в това заявление има само един правоприемник - затова забиваме индекс 0.
        errors.forEach((error: string) => mainApp.addError(error, ErrorLevels.Error));
        isValid = (errors.length == 0) && isValid;

        for (var i = 0; i < mainApp.fields.reorganizeCoOperatives.coOperativeList.length; i++)
            this.matchAppsForReorganizeCooperative(mainApp.fields.reorganizeCoOperatives.coOperativeList[i], a7List);
    ...
    private getErrorsForAppsForSuccessor(successor: F8030_Successor803, additionalApps: ApplicationFormBase[], isInfluxSelected: boolean): string[] {
        let errors = [];

        if (ObjectHelper.isStringNullOrEmpty(successor.subject.indent) && (isInfluxSelected == false)) {
    ...
    private validateAllUICsAreRegisteredInTR(mainApp: V31): boolean {
        let isValid = true;
