import { IResourceManager } from 'Cnsys.Core'
...
class ResourceManager implements IResourceManager {
    ...
    getSystemUnavailableResource(): string {
        var res = this.getResourceByKey('GL_SYSTEM_UNAVAILABLE_E');
        if (res.indexOf(this.defaultMessageForResourceNotFound) >= 0) {
            res = 'Системата е временно недостъпна, моля опитайте отново по-късно.';
        }
