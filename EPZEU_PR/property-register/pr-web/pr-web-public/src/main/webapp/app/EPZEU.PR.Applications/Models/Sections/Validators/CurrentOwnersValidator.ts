...
export class CurrentOwnersValidator extends EPZEUBaseValidator<Owners, IApplicationFormValidationContext> {

  constructor() {
    super();
    this.ruleFor(m => m.persons).notNull();
    this.ruleFor(m => m.propertyDocuments).notNull();
  ...
  public validate(obj: Owners): boolean {
    ...
    if(this.duplicatedIndividualIdentity(obj) || this.duplicatedIndividualBulstat(obj)) {
      obj.addError("duplicatedIdentifierIndividual", this.getMessage("PR_APP_00100_E"));
      isValid = false;
    }

    if(this.duplicatedLegalEntityBulstat(obj)) {
      obj.addError("duplicatedIdentifierLegalEntity", this.getMessage("PR_APP_00102_E"));
      isValid = false;
    }
