...
namespace EPZEU.ServiceLimits.AspNetCore.Mvc
{
    ...
    public class ContextServiceLimiterAttribute : Attribute, IAsyncResourceFilter, IServiceLimiterFilter
    {
        ...
        private async Task OnResourceExecutionInternalAsync(ResourceExecutingContext context, ResourceExecutionDelegate next)
        {
            ...
            IEPZEUUserAccessor userAccessor = context.HttpContext.RequestServices.GetRequiredService<IEPZEUUserAccessor>();
            ...
            StringValues responseTokenString;
        ...
        {
            return EPZEUJsonSerializer.Serialize(token);
        }

        private ContextLimiterToken DeserializeToken(string tokenString)
        {
            return EPZEUJsonSerializer.Deserialize<ContextLimiterToken>(tokenString);
        }

        private ContextLimiterToken CreateToken(ResourceExecutingContext context, IEPZEUUserAccessor userAccessor)
        {
        ...
        private bool IsSkipContextLimiterTokenValid(ResourceExecutingContext context, IEPZEUUserAccessor userAccessor, ContextLimiterToken requestedToken, ILogger logger)
        {
