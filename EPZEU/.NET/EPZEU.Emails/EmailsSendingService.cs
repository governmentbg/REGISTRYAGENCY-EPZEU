...
namespace EPZEU.Emails
{
    ...
    internal class EmailsSendingService : IEmailsSendingService
    {
        ...
        public async Task<IEnumerable<SendingMailResult>> TrySendEmailsAsync(IEnumerable<EmailMessage> mails, CancellationToken cancellationToken)
        {
            ...
            {
                await client.ConnectAsync(_emailsOptions.EP_EML_SMTP_HOST, _emailsOptions.EP_EML_SMTP_PORT, MailKit.Security.SecureSocketOptions.Auto, cancellationToken);

                try
                {
                    if (!string.IsNullOrEmpty(_emailsOptions.EP_EML_SMTP_USER))
                        await client.AuthenticateAsync(_emailsOptions.EP_EML_SMTP_USER, _emailsOptions.EP_EML_SMTP_PASSWORD, cancellationToken);
        ...
        MimeMessage CreateMailMessage(EmailMessage mail)
        {
            ...
            message.From.Add(InternetAddress.Parse(_emailsOptions.EP_EML_SEND_FROM));
