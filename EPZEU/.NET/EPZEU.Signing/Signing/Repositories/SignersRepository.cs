...
namespace EPZEU.Signing.Repositories
{
    ...
    /// </summary>
    internal class SignersRepository : RepositoryBase<Signer, long?, SignerSearchCriteria, SigningDataContext>, ISignersRepository
    {
        #region Constructors
        ...
        {
            long? signerID = await context.SignerCreateAsync(item.Name, item.Ident, item.Order, item.ProcessID, cancellationToken);
        ...
        protected async override Task<IEnumerable<Signer>> SearchInternalAsync(SigningDataContext context, PagedDataState state, SignerSearchCriteria searchCriteria, CancellationToken cancellationToken)
        {
            ...
            using (var reader = res.reader)
            {
                ...
                signers = reader.ReadToList<Signer>();
            }
