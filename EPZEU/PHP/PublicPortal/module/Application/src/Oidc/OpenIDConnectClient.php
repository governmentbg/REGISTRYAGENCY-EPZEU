...
class OpenIDConnectClient
{
     ...
     * @param null $issuer
     */
    public function __construct($provider_url = null, $client_id = null, $client_secret = null, $issuer = null, $config, $cookieDomainName) {
    ...
    public function authenticate() {
        ...
        if (isset($_REQUEST['code'])) {
            ...
            if (isset($token_json->error)) {
                ...
                throw new OpenIDConnectClientException('Got response: ' . $token_json->error .' Code: '.$code);
            }
     ...
     * @throws OpenIDConnectClientException
     */
    public function signOut($accessToken, $redirect, $autoRedirectAfterLogout = false) {
        ...
        if ($autoRedirectAfterLogout)
        	$signout_params['postlogoutregirect'] = 'true';
     ...
     * @throws OpenIDConnectClientException
     */
    public function requestDelegationToken() {
    	$token_endpoint = $this->getProviderConfigValue('token_endpoint');
    ...
    public function refreshToken($refresh_token) {
        ...
        if (isset($json->access_token)) {
            ...
            setcookie('AT', $json->access_token, null, '/', $this->cookieDomainName, true, true);
        }

        if (isset($json->refresh_token)) {
            $this->refreshToken = $json->refresh_token;
            setcookie('RT', $json->refresh_token, null, '/', $this->cookieDomainName, true, true);
        }
    ...
    protected function setNonce($nonce) {
        $this->setCookieKey('openid_connect_nonce', $nonce);
        return $nonce;
    }
    ...
    protected function getNonce() {
        return $this->getCookieKey('openid_connect_nonce');
    }
    ...
    protected function unsetNonce() {
        $this->unsetCookieKey('openid_connect_nonce');
    }
    ...
    protected function setState($state) {
        $this->setCookieKey('openid_connect_state', $state);
        return $state;
    }
    ...
    protected function getState() {
        return $this->getCookieKey('openid_connect_state');
    }
    ...
    protected function unsetState() {
        $this->unsetCookieKey('openid_connect_state');
    }
   ...
   */
   ...
   // TODO - да се изнесе в криптирано куки
	...
	*/
    ...
    protected function getCookieKey($key) {
    	...
    	if (isset($_COOKIE[$key]))
    		$decryptValue = AppService::decrypt($_COOKIE[$key], $this->config['api_key'], $this->config['api_iv']);

    	return $decryptValue;
    }

    protected function setCookieKey($key, $value) {
    	$encryptValue = AppService::encrypt($value, $this->config['api_key'], $this->config['api_iv']);
    	setcookie($key, $encryptValue, null, '/', $this->cookieDomainName, true, true);
    	$_COOKIE[$key] = $encryptValue;
    }

    protected function unsetCookieKey($key) {
    	setcookie($key, '', time() - 3600, '/', $this->cookieDomainName);
    	unset($_COOKIE[$key]);
    }
